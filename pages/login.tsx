...
export default function App() {
  ...
  return (
    <div className="w-screen h-screen flex items-star columns-2 px-16 py-16">
      <div className="mx-6 my-7 w-4/12">
      ...
      <div className="flex h-fit flex-col w-8/12">
        <div className="bg-black/5 px-6 py-6">
          ...
          {serverDetails && (
            <div className="flex flex-col w-full">
              <Input placeholder="Email" value={email} onChange={e => setEmail(e.target.value)} className="w-96" />
              <Input type="password" placeholder="App Password" value={password} onChange={e => setPassword(e.target.value)} className="w-96" />


              <div className="flex flex-row w-full gap-3">
                <Button className="flex items-center justify-center py-6 h-5 w-20" onClick={login} disabled={!canLogin || isLoggingIn}>{isLoggingIn ? 'Logging in...' : 'Login'}</Button>
                <div className="flex flex-col max-w-full">
                  <p className="text-sm font-semibold">Privacy</p>
                  <p className="text-sm opacity-75 mb-1">Skyname does not store any of your Bluesky credentials on its server. Credentials, such as your access token are stored locally and sent to the server for direct verification with your Bluesky server. We only store is your did (identifier), registered username (e.g. example.tired.io) and previous username (e.g. @example.bsky.social). By logging in below, you agree that you're okay with this.</p>
                </div>
